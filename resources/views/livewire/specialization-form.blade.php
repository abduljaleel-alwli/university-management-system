<div>
    ...
    <button wire:click="openModal"
        class="flex items-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-xl shadow-md font-semibold
                            ...
                            whitespace-nowrap btn-bg">{{ __('New Specialization') }}</button>
        ...
        <div class="fixed inset-0 flex items-center justify-center z-50">
            ...
            <div class="bg-white p-6 rounded-lg shadow-lg w-96 z-10">
                <h2 class="text-xl font-semibold mb-4">
                    {{ $specialization_id ? __('Edit Specialization') : __('Add a new Specialization') }}
                </h2>
                ...
                <div class="flex justify-end space-x-2">
                    <button wire:click="$set('modalOpen', false)"
                        class="px-4 py-2 bg-gray-300 rounded-md mx-2">{{ __('Cancel') }}</button>
                    <button wire:click="save" class="px-4 py-2 bg-blue-500 text-white rounded-md btn-bg">
                        {{ $specialization_id ? __('Update') : __('Save') }}
                    </button>
                </div>
            </div>
        </div>
    ...
    @if ($confirmDeleteModal)
        <div class="fixed inset-0 flex items-center justify-center z-50">
            ...
            <div class="bg-white p-6 rounded-lg shadow-lg w-96 z-10">
                ...
                <p class="mb-4">
                    {{ __('Are you sure you want to delete this specialization? This process cannot be undone.') }}</p>

                <div class="flex justify-end space-x-2">
                    <button wire:click="$set('confirmDeleteModal', false)"
                        class="px-4 py-2 bg-gray-300 rounded-md">{{ __('Cancel') }}</button>
                    <button wire:click="deleteSpecialization"
