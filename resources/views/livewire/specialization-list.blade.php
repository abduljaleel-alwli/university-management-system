<div class="bg-white p-6 rounded-lg shadow-soft-xl mt-6 space-y-4">
    ...
    <div class="flex mb-4 gap-2">
        <div class="flex items-center gap-2 w-full md:w-auto">
            ...
            <button wire:click="searchSpecializations" wire:loading.attr="disabled"
                ...
                <span>{{ __('Search') }}</span>
                <span wire:loading wire:target="searchSpecializations">
            ...
            <input type="text" wire:model="search" placeholder="{{ __('Search a Specialization') }}..."
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($specializations as $index => $specialization)
                    <tr class="hover:bg-gray-50 transition duration-200">
                        ...
                        <td class="px-6 py-4 text-sm text-center text-gray-600">{{ $specialization->name_ar }}</td>
                        <td class="px-6 py-4 text-sm text-center text-gray-600">{{ $specialization->name_en }}</td>
                        <td class="px-6 py-4 text-sm">
                            <div class="flex items-center space-x-2" style="justify-content: center">
                                <button
                                    wire:click="$dispatch('openModal', { specializationId: {{ $specialization->id }} })"
                                    class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded-lg text-sm transition duration-200 mx-2">
                                    {{ __('Edit') }}
                                </button>
                                <button
                                    wire:click="$dispatch('confirmDelete', { specializationId: {{ $specialization->id }} })"
